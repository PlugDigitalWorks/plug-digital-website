...
async function getCategoryBySlug(slug: string): Promise<Category | null> {
  try {
    ...
    if (!category || category.type !== 'WATCH') return null;
...
async function getWatchesByCategorySlug(
  categorySlug: string,
  page: number = 1,
  search?: string,
  sort?: string,
  ...
  try {
    ...
    const data = (await response.json()) as WatchesResponse;
...
export async function generateMetadata({
  ...
  if (!category) {
    return {
      title: 'Watches | Pacha London',
      description: 'Discover luxury watches at Pacha London.',
    };
  }

  const title = category.seoTitle || `${category.name} Watches | Pacha London`;

  const description =
    category.seoDescription ||
    category.description ||
    `Discover our collection of ${category.name.toLowerCase()} watches at Pacha London.`;
  ...
  return {
    ...
    alternates: {
      canonical: `${BASE_URL}/watch/${category.slug}`,
    },
  };
}
...
export default async function WatchCategoryPage({
  ...
  // 2️⃣ Watches fetch (category filter + pagination + filters)
  const {
    products: watches,
    allBrands,
    allCategories,
    pagination,
  } = await getWatchesByCategorySlug(category.slug!, page, search, sort, brand);
